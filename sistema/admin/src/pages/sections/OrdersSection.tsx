...
const STATUS_TIMELINE = ['PENDING', 'CONFIRMED', 'PICKING', 'CONFERENCE_PENDING', 'PACKING', 'READY_FOR_CHECKOUT', 'READY_FOR_DELIVERY', 'OUT_FOR_DELIVERY', 'DELIVERED', 'COMPLETED'] as const
const STATUS_TIMELINE_PICKUP = ['PENDING', 'CONFIRMED', 'PICKING', 'CONFERENCE_PENDING', 'PACKING', 'READY_FOR_CHECKOUT', 'READY_FOR_PICKUP', 'COMPLETED'] as const
            ...
            <div className="border-t border-[#f1dbe3] bg-white px-6 py-4 flex justify-between rounded-b-2xl">
              <Button
                ...
                onClick={() => {
                  ...
                  // JON-136 (Auditoria 360, High): nome/observacao/endereco/nome
                  // de produto vem de texto que o CLIENTE digita -- sem escapeHtml
                  // aqui, um pedido com "<img onerror=...>" no nome ou nas
                  // observacoes virava HTML executavel na origem do admin assim
                  // que o operador clicasse Imprimir (JWT do admin fica em
                  // localStorage, mesma origem). Cada valor livre passa por
                  // escapeHtml antes de entrar na string.
                  const custName = escapeHtml(order.customer?.name || '-')
                  const custWhatsapp = escapeHtml(order.customer?.whatsapp || '-')
                  const notes = order.notes ? escapeHtml(order.notes) : ''
                  const addrHtml = addr
                    ? `<h2>Endereço de Entrega</h2><p>${escapeHtml(addr.street || '')}${addr.number ? ', ' + escapeHtml(addr.number) : ''}${addr.complement ? ' - ' + escapeHtml(addr.complement) : ''}<br>${escapeHtml([addr.neighborhood, addr.city, addr.state].filter(Boolean).join(' - '))}${addr.zipCode ? ' - CEP ' + escapeHtml(addr.zipCode) : ''}${addr.reference ? '<br>Ref: ' + escapeHtml(addr.reference) : ''}</p>`
                    : ''
                  const itemsHtml = (order.items || [])
                    .map(
                      (i) =>
                        `<tr><td>${escapeHtml(i.product?.name || i.productId)}</td><td>${i.requestedQuantity ?? i.quantity}</td><td>${i.fulfilledQuantity ?? i.quantity}</td><td>${getItemStatusLabel(i.status) || 'Pendente'}</td><td>R$ ${(Number(i.finalSubtotal) || i.subtotal).toFixed(2)}</td></tr>`,
                    )
                    .join('')
                  const html = `<!DOCTYPE html><html><head><meta charset="utf-8"><title>Pedido #${order.id.slice(-8).toUpperCase()}</title><style>*{margin:0;padding:0;box-sizing:border-box}body{font-family:Arial,sans-serif;padding:20px;font-size:12px;color:#333}h1{font-size:16px;margin-bottom:4px}h2{font-size:13px;margin:12px 0 4px;border-bottom:1px solid #ccc;padding-bottom:2px}table{width:100%;border-collapse:collapse;margin-top:4px}th,td{border:1px solid #ddd;padding:4px 6px;text-align:left;font-size:11px}th{background:#f5f5f5;font-weight:bold}.total{text-align:right;font-weight:bold;font-size:13px;margin-top:8px}.meta{display:flex;gap:24px;flex-wrap:wrap;margin:6px 0}.meta span{font-size:11px}@media print{body{padding:10px}}</style></head><body><h1>Pedido #${order.id.slice(-8).toUpperCase()}</h1><div class="meta"><span><b>Cliente:</b> ${custName}</span><span><b>WhatsApp:</b> ${custWhatsapp}</span><span><b>Data:</b> ${new Date(order.createdAt).toLocaleString('pt-BR')}</span><span><b>Status:</b> ${orderStatusLabels[order.status] || order.status}</span><span><b>Pagamento:</b> ${order.paymentMethod ? getPaymentMethodLabel(order.paymentMethod) : '-'}</span></div>${addrHtml}${notes ? `<h2>Observacoes</h2><p>${notes}</p>` : ''}<h2>Itens</h2><table><thead><tr><th>Produto</th><th>Qtd</th><th>Qtd Final</th><th>Status</th><th>Subtotal</th></tr></thead><tbody>${itemsHtml}</tbody></table><div class="total">Subtotal: R$ ${order.subtotal.toFixed(2)}${order.discount > 0 ? ' | Desconto: -R$ ' + order.discount.toFixed(2) : ''}${order.delivery > 0 ? ' | Frete: R$ ' + order.delivery.toFixed(2) : ''} | <b>Total: R$ ${order.total.toFixed(2)}</b></div></body></html>`
                  // noopener: janela de impressao nao precisa (e nao deveria)
                  // ter acesso de volta a esta aba do admin.
